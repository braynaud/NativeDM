...
public class LoginActivity extends Activity {
	...
	/**
	 * Declare the PARSE CLIENT KEY and APPLICATION ID for the Parse
	 * Initialization
	 */
	 ...
	 * 
	 * @param applicationId
	 *            , clientID are the application id and client id for the parse
	 *            initialization
	 */
		...
		ParseTwitterUtils.logIn(this, new LogInCallback() {
			public void done(ParseUser user, ParseException err) {
				...
				if (user == null) {
					...
					// After someone has been signed up, link their accounts to
					// parse using the
					// TwitterUtils.link to make sure they are linked to their
					// account
					linkTwitterUser();
	 ...
	 * 
	 * @param facebookConsumerKey
	 *            , facebookConsumerSecret
	 */

	/*
	 * public void facebookCallback(View v) { //Get the users inputed USERNAME
	 * and PASSWORD from the login page String username = getLoginUserName();
	 * String password = getLoginPwd();
	 * 
	 * //Verify that the usernames and password are valid before you register
	 * the user if (username.length() < 1) { invalidUsernameAlert();
	 * dialog.dismiss(); return; } if (password.length() <1){
	 * invalidPasswordAlert(); dialog.dismiss(); return; }
	 * ParseTwitterUtils.logIn(this, new LogInCallback() {
	 * 
	 * @Override public void done(ParseUser user, ParseException err) { if (user
	 * == null) { listener.onError("facebook", err); } else if (user.isNew()) {
	 * listener.onSignup("facebook", user); } else {
	 * listener.onSignin("facebook", user); } } }); }
	 */

	/**
	 * Use getters to retrieve the usernames and passwords getLoginUserName gets
	 * the name of the user getLoginPwd gets the user's password
	 */
	...
	public void loginCallback(View v) {
		...
		// Verify that the user names and password are valid before you register
		// the user
		...
		ParseUser.logInInBackground(getLoginUserName(), getLoginPwd(),
				new LogInCallback() {
					...
					public void done(ParseUser user, ParseException e) {
						...
						// Otherwise if the user is new and doesnt have an
						// account
						// Create an account directly to parse
						else {
							// Create a log in failed alert to show the user
							// that login failed
							loginFailedAlert();
	...
	public void linkTwitterUser() {
		// Check and see if the user is linked to the account. If he or she is
		// not linked, make the link
	...
	public void signupCallback(View v) {
		final View view = v;
		// Display a sign up alert to notify the user that they are being signed
		// up
		createSignUpAlert();
		...
		newUser.signUpInBackground(new SignUpCallback() {
			public void done(ParseException e) {
				...
				if (e == null) {
					...
					signedUpSuccessAlert();
					loginProgress();
					// Give the device sometime to update the info in parse.com.
					...
					handler.postDelayed(new Runnable() {
						@Override
						public void run() {
							// Give the device 5 seconds to delay as we wait for
							// the timer to update.
							// Once that is done, prompt the device to log in
							// the user.
							loginCallback(view);
	...
	public void loginProgress() {
		ProgressDialog dialog = ProgressDialog.show(this, "", "Logging in ..."
				+ "This may take a few seconds as we update your information",
				true);
	}
	...
	/**
	 * This is a method that loads a new activity after successfully login in to
	 * the system
	 * 
	 */
	...
	/**
	 * This is a dialog builder that will be used to send alerts to the user in
	 * case they have any issue during sign up or login
	 */
